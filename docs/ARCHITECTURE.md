...
- **Tag `v1.0.0` on main** when the M4 PR merges. V2 features continue the same pattern (`feat/monte-carlo`, etc.).
- No `develop` branch, no gitflow — unnecessary for a solo project.
- CI (GitHub Actions, added in M0): `cargo fmt --check`, `clippy -D warnings`, `cargo test`, `tsc --noEmit` as required checks on PRs to main.
